...
export async function handleAiAgentResponse(conversationId: string, inboundMessage: string): Promise<AiAgentResult> {
    try {
        ...
        // Check if ecommerce integration is active for this company
        const ecommerceIntegration = await prisma.ecommerceIntegration.findUnique({
            where: { companyId: conversation.companyId },
            select: { active: true },
        });
        const ecommerceEnabled = !!ecommerceIntegration?.active;
